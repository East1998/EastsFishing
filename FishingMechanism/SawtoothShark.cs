...
namespace EastsFishing.FishingMechanism
{
	public class SawtoothShark : ModNPC
	{
        ...
        public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
            {
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/SawtoothSharkGore1"), 1f);
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/SawtoothSharkGore2"), 1f);
            }
		}
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			for (int i = 0; i < 1; ++i)
		    {
				...
				{
				    return SpawnCondition.OceanMonster.Chance / 180;	
				}
				else
				{
				    return SpawnCondition.OceanMonster.Chance / 200;
				}
			}
			return SpawnCondition.OceanMonster.Chance / 200;
		}
