...
namespace EastsFishing.FishingMechanism
{
	public class Tier3Fish : ModNPC
	{
        ...
        public override void HitEffect(int hitDirection, double damage)
		{
			if (npc.life <= 0)
            {
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/Tier3FishGore1"), 1f);
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/Tier3FishGore2"), 1f);
            }
		}
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			for (int i = 0; i < 1; ++i)
		    {
				...
				{
				    return SpawnCondition.WaterCritter.Chance * 220;	
				}
				else
				{
